                            ...
                            <table id="-table" class="row-border data-table-filter table-style">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{get_names('key_point', $course_version->details['new_learn_key_points'])}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{_('Description')}}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{__(get_names('subcategory', [$course_version->details['subcategory_id']]))}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{_('Tags')}}</td>
                                    ...
                                    <tr>
                                        <td>{{_('Language')}}</td>
                                        <td>{{get_names("course_langauge", [$course->course_language_id])}}</td>
                                        <td>{{get_names('course_langauge', [$course_version->details['course_language_id']])}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{_('Difficulty Level')}}</td>
                                        <td>{{get_names("difficulty_level", [$course->difficulty_level_id])}}</td>
                                        <td>{{get_names('difficulty_level', [$course_version->details['difficulty_level_id']])}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{_('Therapy Tutorial Thumbnail')}}</td>
                                        <td><img src="{{getImageFile($course->image)}}" width="200"></td>
                                        <td><img src="{{getImageFile($course_version->details['image'])}}" width="200"></td>
                                    </tr>
                                    <tr>
                                            ...
                                            <div class="col-md-12 mb-30  videoSource">
                                                <div class="video-player-area ">
                                                    <video id="player" playsinline controls
                                                        ...
                                                        controlsList="nodownload">
                                                        <source src="{{ getVideoFile(@$course_version->details['video']) }}"
                                                            type="video/mp4" >
                                                    </video>
                                                </div>
                                            </div>
                                    ...
                                    <tr>
                                        <td>{{_('Lessons')}}</td>
                                        <td style="max-width: 200px">{{implode(", ", $course->lessons->pluck('name')->toArray())}}</td>
                                        <td style="max-width: 200px">{{get_names('lesson', $course_version->details['tags'])}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{_('Course Instructors')}}</td>
                                        <td >
                                            @foreach($course->course_instructors as $instructor)
                                            {{$instructor->user->name}} | {{_('Share')}} : {{$instructor->share}} ,
                                            @endforeach
                                        </td>
                                        <td >
                                            @foreach ($course_version->details['course_instructors'] as $instructor)
                                            {{get_names('user', [$instructor['instructor_id']])}} | {{_('Share')}} : {{$instructor['share']}} ,
