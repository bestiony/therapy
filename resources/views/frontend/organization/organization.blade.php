...
<script>
    const paginateRoute = "{{ route('organizations_more') }}";
    const filterRoute = "{{ route('filter.organizations') }}";
    const maxPrice = "{{ $priceMax }}";
    const mapData =  @json($mapData);
    // L.mapbox.accessToken = "{{ get_option('map_api_key') }}";
    // L.mapbox.accessToken = "{{ env('MAP_API_KEY') }}";
</script>
...
<script>
    ...
    document.getElementById("exampleRadios" + category_id).checked = true;
    filterData()
</script>
@endpush
