        ...
        <table class="table bg-white my-courses-page-table stu-consult-tbl">
            ...
            @foreach($orderItems as $orderItem)
                ...
                <tr>
                    ...
                    <td class="wishlist-price font-15 color-heading text-center">
                        @if(@$orderItem->bookingHistory->status == 1)
                            <span class="status active">{{ __('Approved') }}</span>
                            @if($orderItem->order->conversation)
                            <div class="red-blue-action-btns text-center my-3">
                                <button type="button"
                                    data-route="{{ route('student.messages', ['convo'=>$orderItem->order->conversation->id]) }}"
                                    class="px-2 py-2 btn btn-success default-hover-btn joinChat ">{{ __('Join Chat') }}</button>
                            </div>
                            @endif
