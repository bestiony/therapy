...
@section('content')
    ...
    @push('script')
        @livewireScripts
    @endpush
    <div class="inner-container " style="height: 100% !important;">
        <div class="row flex-wrap vh-100">
            ...
            {{-- <section id="messages" class="chat col-12 col-sm-8  vh-100 align-items-end">
                        ...
                        <li>
                            @if (isset($current_conversation))
                                @if ($current_conversation->status == 'active')
                                    <form action="{{ route('instructor.stop-conversation') }}" method="POST">
                                        @csrf
                                ...
                                @else
                                    <form action="{{ route('instructor.resume-conversation') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="conversation_id" value="{{ $selected_conversation }}">
                ...
                @if (isset($current_conversation))
                    @if ($current_conversation->status == 'active')
                        <form enctype="multipart/form-data" action="{{ route('instructor.send_message') }}" method="POST"
                            class="">
                            @csrf
