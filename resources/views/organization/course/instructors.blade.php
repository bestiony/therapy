                            ...
                            <form method="POST"
                                ...
                                <div class="upload-course-step-item upload-course-overview-step-item">
                                    <div class="row">
                                        ...
                                        @php
                                            $instructors_id = @$course->course_instructors->pluck('instructor_id')->toArray();
                                        @endphp
                                        <div class="col-md-12" {{-- style="display:none;" --}}>
                                            <label
                                                class="label-text-title color-heading font-medium font-16 mb-3">{{ __('Other Instructors') }}
                                                <span
                                            ...
                                            <select class="select2 mb-3" id="instructor-id" multiple>
                                                @foreach ($instructors as $instructor)
                                                    ...
                                                    <option value="{{ $instructor->id }}"
                                                        @if ($oldData == $instructor->id) selected
                                                            ...
                                                            <tr class="instructor-child">
                                                                ...
                                                                <td>
                                                                    <input type="number" min=0 max=100
                                                                        name="share[{{ $instructor->id }}]"
                                                                        value="{{ $oldShare }}" class="form-control"
                                                                        placeholder="Type revenew share parcentage"
                                                                        required>
                                                                </td>
                                                            </tr>
                                                        @endif
                                ...
                                <div class="stepper-action-btns">
                                    <a href="{{ route('organization.course.add-lessons', ['uuid' => $course->uuid]) }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('select2:unselecting', '#instructor-id', function(e) {
                $(document).find('input[name^=instructor_id][value=' + e.params.args.data.id + ']').closest(
                    '.instructor-child').remove();
