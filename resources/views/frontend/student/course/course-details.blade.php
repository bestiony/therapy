                                    ...
                                    <ul class="course-watch-banner-items">
                                        <li
                                            ...
                                            <span class="iconify me-2"
                                                data-icon="carbon:video"></span>{{ @$course->lectures->count() }}
                                            {{ __('Lectures') }}
                                        </li>
                                        <li
                                            class="font-14 font-medium color-heading {{ $course->course_type == COURSE_TYPE_GENERAL ? '' : 'd-none' }}">
                                            <span class="iconify me-2"
                                                data-icon="akar-icons:book"></span>{{ @$course->lessons->count() }}
                                            {{ __('sections') }}
                                        </li>
                                    ...
                                    <div class="barras mb-4">
                                        <div class="progress-bar-box">
                                            <div
                                                ...
                                                <span
                                                    class="font-15">{{ number_format(studentCourseProgress(@$course->id, @$enrollment->id), 2) }}%
                                                    {{ __('Done') }}</span>
                                            </div>
                                            <div class="barra">
                                                <div class="barra-nivel"
                                                    data-nivel="{{ studentCourseProgress(@$course->id, @$enrollment->id) }}%">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="col-12 col-md-12 col-lg-12 col-xl-7 col-xxl-8">
                            @if (@$lecture_type)
                                ...
                                @if (@$lecture_type == 'youtube')
                                    <div class="video-player-area">
                                        <div class="plyr__video-embed" id="playerVideoYoutube">
                                            <iframe src="https://www.youtube.com/embed/{{ @$youtube_video_src }}"
                                ...
                                @elseif(@$lecture_type == 'video')
                                    <div class="video-player-area">
                                        <video id="player" class="myVideo" playsinline controls
                            ...
                            @else
                                @if ($course->intro_video_check == 1 && getVideoFile($course->video))
                                    <div class="video-player-area">
                                        <video id="player1" playsinline controls
                                            data-poster="{{ getImageFile($course->image) }}" controlsList="nodownload">
                                ...
                                @else
                                    <div class="course-watch-no-video-img">
                                        <img src="{{ getImageFile($course->image) }}" alt=""
                            ...
                            <div class="d-flex justify-content-center">
                                ...
                                <a  class="bg-transparent theme-btn color-heading mb-3"
                                onclick="callCompleteCourse()">{{ __('Finish Segment') }}
                            </a>
                        </div>
                        </div>
                                    ...
                                    <div class="review-progress-bar-wrap">
                                        ...
                                        <div class="barras mb-4">
                                            <div class="progress-bar-box">
                                                <div
                                                    class="progress-hint-value font-14 color-heading d-flex justify-content-between">
                                                    <p class="font-20 font-medium color-heading">{{ __('Course Content') }}
                                                    </p>
                                                    <span
                                                        class="font-15">{{ number_format(studentCourseProgress(@$course->id, @$enrollment->id), 2) }}%
                                                        Done</span>
                                            ...
                                            @forelse(@$course->lessons as $key => $lesson)
                                                <div class="accordion-item">
                                                        ...
                                                        <button
                                                            ...
                                                            <span class="watch-course-title-right-btns d-flex">
                                                                <span class="font-12 color-gray2 mx-2"><span
                                                                        ...
                                                                        data-icon="akar-icons:clock"></span>{{ count($lesson->lectures->whereNotNull('course_id')) }}
                                                                    lectures</span>
                                                                <span class="font-12 color-gray2 mx-2"><span
                                                                        class="iconify font-16 color-orange me-1"
                                                    ...
                                                    <div id="collapse{{ $key }}"
                                                        ...
                                                        @if (checkStudentCourseView($course_lecture_views, $course->id, $lecture->id)) watchFinishedCourse
                                                        @elseif($lecture->id == @$lecture_id_check)
                                                        watchContinuingCourse @endif
                                                        d-flex align-items-center justify-content-between font-15">
                                                                        <div
                                                                            class="play-list-left d-flex align-items-center">
                                                                            <div>
                                                                        ...
                                                                        <div class="play-list-right d-flex">
                                                                            @if ($lock)
                                                                                <span class="video-time-count"><span
                                                                                        class="iconify me-5"
                                                                                        data-icon="ant-design:lock-outlined"></span>{{ $lecture->file_duration }}</span>
                                                                            @else
                                                                                <span
                                                                                    class="video-time-count">{{ $lecture->file_duration }}</span>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                    <!-- Play List Item End-->
                ...
                @else
                    <div class="row">
                        <div class="col-12 col-md-12 col-lg-12">
                            <iframe id="scorm_player" class="scorm-content" frameBorder="0"
                                src="{{ asset('scorm/' . $course->scorm_course->uuid . '/' . $course->scorm_course->entry_url) }}"
                                ...
                                <div class="course-watch-enrolled-wrap d-flex">
                                    <ul>
                                        @foreach ($enrolled_students as $enrolled_student)
                                            <li><img src="{{ getImageFile(@$enrolled_student->user->image_path) }}"
                                                    alt=""></li>
                                        @endforeach
                                    </ul>
                                    <div class="enrolled-count font-12 ms-2"><span
                                            class="color-heading font-medium">{{ $total_enrolled_students }}</span> <span
                            ...
                            <div class="course-tab-nav-wrap course-details-tab-nav-wrap d-flex justify-content-between">
                                <ul class="nav nav-tabs tab-nav-list border-0" id="myTab" role="tablist">
                                    ...
                                    <li style="display:none;" class="nav-item" role="presentation">
                                        <a class="nav-link text-uppercase {{ $action_type ? 'active' : '' }}"
                                            id="Quiz-tab" data-bs-toggle="tab" href="#Quiz" role="tab"
                                            aria-controls="Quiz" aria-selected="false">{{ __('Quiz') }}</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer d-flex justify-content-between align-items-center">
                        <button type="button" class="theme-btn theme-button3"
                            data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                        <button type="button"
                            class="theme-btn theme-button1 submitReview">{{ __('Submit review') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        @if ($course->course_type == COURSE_TYPE_SCORM)
            ...
            function timerHandler() {
                duration++;
                if (duration > 30) {
                    $.post("{{ route('student.my-course.completed_duration', $course->id) }}", {
                        duration: duration,
