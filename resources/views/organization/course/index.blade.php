                        ...
                        <div class="card course-item instructor-my-course-item bg-white">
                            <div class="course-img-wrap flex-shrink-0 overflow-hidden">
                                ...
                                @elseif($course->status == 2)
                                    <span
                                        class="course-tag badge publish-badge radius-3 font-14 font-medium position-absolute">{{ __('Waiting for Review') }}</span>
                                @elseif($course->status == 3)
                                    <span
                                        class="course-tag badge unpublish-badge radius-3 font-14 font-medium position-absolute">{{ __('Hold') }}</span>
                                @elseif($course->status == 4)
                                    <span
                                        class="course-tag badge unpublish-badge radius-3 font-14 font-medium position-absolute">{{ __('Draft') }}</span>
                                @elseif($course->status == 5)
                                    <span
                                        class="course-tag badge unpublish-badge radius-3 font-14 font-medium position-absolute">{{ __('Empty') }}</span>
                                @else
                                    <span
                                        class="course-tag badge unpublish-badge radius-3 font-14 font-medium position-absolute">{{ __('Pending') }}</span>
                                @endif
                                ...
                                <div class="instructor-courses-info-duration-wrap">
                                    <ul class="d-flex align-items-center justify-content-between">
                                        ...
                                        <li class="font-medium font-12"><span class="iconify"
                                                ...
                                                class="instructor-courses-info-duration-wrap-text font-medium color-heading">({{ @$course->VideoDuration }})</span>
                                        </li>
                                        <li class="font-medium font-12"><span class="iconify"
                                                data-icon="carbon:user-multiple"></span>Enrolled<span
                                                class="instructor-courses-info-duration-wrap-text font-medium color-heading">({{ courseStudents($course->id) }})</span>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div class="instructor-my-course-item-left">
                                    ...
                                    <div class="course-item-bottom">
                                        <div class="course-rating d-flex align-items-center">
                                            <span
                                                class="font-medium font-14">{{ number_format($course->average_rating, 1) }}</span>
                                        ...
                                        <div class="instructor-my-courses-btns d-inline-flex">
                                            <a href="{{ route('organization.course.edit', [$course->uuid]) }}"
                                                class="para-color font-14 font-medium d-flex align-items-center"><span
                                                    class="iconify" data-icon="bx:bx-edit"></span>Edit</a>
                                            ...
                                            @if ($course->user_id == auth()->id())
                                                <button
                                                    ...
                                                    <span class="iconify"
                                                        data-icon="ant-design:delete-outlined"></span>Delete
                                                </button>

                                                <form action="{{ route('organization.course.delete', [$course->uuid]) }}"
                            ...
                            <div class="instructor-my-course-btns">
                                <a href="{{ route('organization.resource.index', [$course->uuid]) }}"
                                    class="theme-button theme-button1 instructor-course-btn">{{ __('Resources') }}</a>
                                <a href="{{ route('organization.exam.index', [$course->uuid]) }}"
                                    class="theme-button theme-button1 instructor-course-btn">{{ __('Quiz') }}</a>
                                <a href="{{ route('organization.assignment.index', [$course->uuid]) }}"
