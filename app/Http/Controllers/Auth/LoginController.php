...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        $this->middleware('guest')->except(['logout','verifyPhone']);
    }
        ...
        */
        if (Auth::attempt($credentials)) {
            ...
            if (Auth::user()->is_admin())
            {
                ...
                $route = $this->redirectToVerifyPhoneIfNotVerified();
    ...
    public function redirectToVerifyPhoneIfNotVerified(){
        if (get_option('registration_phone_number_verification') == 1){
        if(Auth::user()->phone_is_verified == 0){
    ...
    public function verifyPhone(Request $request){
        ...
        if ($verification->valid) {
            // $user = tap(User::where('phone_number', $data['phone_number']))->update(['phone_is_verified' => true]);
            /* Authenticate user */
            ...
            $user->update();
            $this->showToastrMessage('success', __('You Phone Number Was Confirmed Successfully'));

            // return redirect(RouteServiceProvider::HOME);
            toastr()->success(__('You Phone Number Was Confirmed Successfully'),'', ["positionClass" => "toast-bottom-right"]);
            return redirect()->route('home')->with(['message' => 'You Phone Number Was Confirmed Successfully']);
        }
