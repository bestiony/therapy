...
if (!function_exists('selectStatement')) {
    function selectStatement()
    {
        ...
        case when org.id is null then ins.monthly_rate else org.monthly_rate end as monthly_rate,
        case when org.id is null then ins.hours_per_month else org.hours_per_month end as hours_per_month,
        case when org.id is null then ins.monthly_old_rate else org.monthly_old_rate end as monthly_old_rate,
...
if (!function_exists('orgSelectStatement')) {
    function orgSelectStatement()
    {
        return 'case when org.id is null then org.uuid end as uuid,
        case when org.id is null then org.country_id end as country_id,
        case when org.id is null then org.province_id end as province_id,
        case when org.id is null then org.state_id end as state_id,
        case when org.id is null then org.city_id end as city_id,
        case when org.id is null then org.professional_title end as professional_title,
        case when org.id is null then org.postal_code end as postal_code,
        case when org.id is null then org.address end as address,
        case when org.id is null then org.about_me end as about_me,
        case when org.id is null then org.gender end as gender,
        case when org.id is null then org.social_link end as social_link,
        case when org.id is null then org.slug end as slug,
        case when org.id is null then org.is_private end as is_private,
        case when org.id is null then org.remove_from_web_search end as remove_from_web_search,
        case when org.id is null then org.is_offline end as is_offline,
        case when org.id is null then org.offline_message end as offline_message,
        case when org.id is null then org.consultation_available end as consultation_available,
        case when org.id is null then org.hourly_rate end as hourly_rate,
        case when org.id is null then org.hourly_old_rate end as hourly_old_rate,
        case when org.id is null then org.monthly_rate end as monthly_rate,
        case when org.id is null then org.hours_per_month end as hours_per_month,
        case when org.id is null then org.monthly_old_rate end as monthly_old_rate,
        case when org.id is null then org.available_type end as available_type,
        case when org.id is null then org.created_at end as approval_date';
    }
}
