    ...
    <div class="instructor-profile-right-part">
        <form method="POST" action="{{ route('save.profile', [$instructor->uuid]) }}" enctype="multipart/form-data">
                        ...
                        <select name="skills[]" class="form-control select2" multiple>
                            @foreach ($skills as $skill)
                                <option value="{{ $skill->id }}"
                                    {{ in_array($skill->id, $instructor->skills->pluck('id')->toArray()) ? 'selected' : '' }}>
                        ...
                        <select name="languages[]" class="form-control select2" multiple>
                            @foreach ($languages as $language)
                                <option value="{{ $language->id }}"
                                    {{ in_array($language->id, $instructor->user->languages ?? []) ? 'selected' : '' }}>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-12 mb-30">
                    <input type="radio" {{ $instructor->intro_video_check == 1 ? 'checked' : '' }} id="video_check"
                        class="intro_video_check" name="intro_video_check" value="1">
                    <label for="video_check">{{ __('Video Upload') }}</label><br>
                    <input type="radio" {{ $instructor->intro_video_check == 2 ? 'checked' : '' }} id="youtube_check"
                ...
                <div class="col-md-12 mb-30">
                    ...
                    <input type="text" name="youtube_video_id" id="youtube_video_id"
                        placeholder="{{ __('Type your youtube video ID') }}" value="{{ $instructor->youtube_video_id }}"
                    ...
                    <div class="col-md-12 mb-30 d-none videoSource">
                        <div class="video-player-area ">
                            <video id="player" playsinline controls data-poster="{{ getImageFile(@$instructor->image) }}"
                                ...
                                <source src="{{ getVideoFile(@$instructor->video) }}" type="video/mp4">
                            </video>
                        </div>
                    </div>
                @endif
                @if ($instructor->youtube_video_id)
                    <div class="col-md-12 mb-30 d-none videoSourceYoutube">
                        <div class="video-player-area ">
                            <div class="plyr__video-embed" id="playerVideoYoutube">
                                <iframe src="https://www.youtube.com/embed/{{ @$instructor->youtube_video_id }}"
                        ...
                        @if ($instructor->certificates)
                            @foreach ($instructor->certificates as $certificate)
                                <div class="row mb-30 removable-item">
                                    <div class="col-md-8">
                                        <label
                                            class="font-medium font-15 color-heading">{{ __('Title of the Certificate') }}</label>
                                        <input type="text" name="certificate_title[]"
                                            value="{{ $certificate->name }}" class="form-control"
                            ...
                            @foreach ($instructor->awards as $award)
                                <div class="instructor-add-extra-field-box mb-30 removable-item">
                                    <div class="row">
                                        <div class="col-md-8">
                                            <label
                                                class="font-medium font-15 color-heading">{{ __('Title of the Award') }}</label>
...
@push('script')
    ...
    <script>
        "use strict"
        $(function (){
            var intro_video_check = "{{ $instructor->intro_video_check }}";
