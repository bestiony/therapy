...
class CertifiedParentController extends Controller
{
    use General, ImageSaveTrait;

    protected $certifiedParentModel, $studentModel;
    public function __construct(CertifiedParent $instructor, Student $student)
    {
        $this->certifiedParentModel = new Crud($instructor);
    ...
    public function index()
    {
        ...
        $data['subNavCertifiedParentIndexActiveClass'] = 'active';
        $data['certified_parents'] = CertifiedParent::query()
            ->with('user')
            ...
            ->paginate(10);
        return view('organization.certified_parent.index', $data);
    }

    public function create()
    {
        $data['title'] = 'Add CertifiedParent';
        $data['navCertifiedParentActiveClass'] = 'has-open';
        $data['subNavCertifiedParentAddActiveClass'] = 'active';
        ...
        return view('organization.certified_parent.create', $data);
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'phone_number' => 'bail|numeric|digits_between:8,14|unique:users,mobile_number',
            'address' => 'required',
            'gender' => 'required',
            'about_me' => 'required',
        ...
        if (CertifiedParent::where('slug', getSlug($user->name))->count() > 0) {
            $slug = getSlug($user->name) . '-' . rand(100000, 999999);
        } else {
            $slug = getSlug($user->name);
        }

        $certified_parent_data = [
            'user_id' => $user->id,
            'first_name' => $request->first_name,
            'last_name' => $request->last_name,
            'address' => $request->address,
        ...
        $this->certifiedParentModel->create($certified_parent_data);
        ...
        $this->showToastrMessage('success', __('CertifiedParent created successfully'));
        return redirect()->route('organization.certified_parent.index');
    }

    public function edit($uuid)
    {
        $data['title'] = 'Edit CertifiedParent';
        $data['navCertifiedParentActiveClass'] = 'has-open';
        $data['subNavCertifiedParentIndexActiveClass'] = 'active';
        $data['categories'] = Category::active()->orderBy('name', 'asc')->select('id', 'name')->get();

        $data['instructor'] = CertifiedParent::where('organization_id', auth()->user()->organization->id)->where('uuid', $uuid)->firstOrFail();
        if ($data['instructor'] == null) {
            $this->showToastrMessage('error', __('CertifiedParent Not Found!'));
    ...
    {
        $instructor = $this->certifiedParentModel->getRecordByUuid($uuid);
        ...
        $request->validate([
            ...
            // 'area_code' => 'required',
            'user_category_id' => 'required|exists:categories,id',
            'phone_number' => 'bail|numeric|digits_between:8,14|unique:users,mobile_number,' . $instructor->user_id,
            'address' => 'required',
            'gender' => 'required',
            'about_me' => 'required',
            'image' => 'mimes:jpeg,png,jpg|file|dimensions:min_width=300,min_height=300,max_width=300,max_height=300|max:1024'
    ...
    public function status(Request $request)
    {
        ...
        $instructor = CertifiedParent::where('organization_id', auth()->user()->organization->id)->findOrFail($request->id);
        if (is_null($instructor)) {
            return response()->json(['message' => __('CertifiedParent Not Found!'), 'status' => false]);
        }
        $instructor->status = $request->status;
        $instructor->save();
        return response()->json(['message' => __('CertifiedParent status has been updated'), 'status' => true]);
    }
